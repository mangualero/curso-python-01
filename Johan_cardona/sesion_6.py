...
def numBooksByKey(key):
    allItems = []

    for i in range(len(books)):
        allItems.append(books[i][key])

    numBooksPerKey = { i: allItems.count(i) for i in allItems }

    return numBooksPerKey

# print(numBooksByKey("category"))
#  Por categoria, ej: 
...
def getBooksByKey(key):
    items = {}

    for book in books:
        if items.get(book[key]):
            items[book[key]].append(book)
        else:
            items[book[key]] = [book]

    print(items)

print(getBooksByKey("title"))

print(type(getBooksByKey))
